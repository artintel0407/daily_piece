const pieces = [
  {
    id: 1,
    day: 1,
    date: "2026-05-25",
    title: "Daily Pieces 프로젝트 기획",
    category: "Planning",
    summary: "매일 만든 작은 결과물을 카드처럼 쌓는 개인 작업 아카이브 웹사이트의 방향성을 정했다.",
    thumbnailText: "Daily\nPieces",
    description:
      "오늘은 Daily Pieces라는 프로젝트를 시작했다. 이 웹사이트는 감성적인 작업, 생산적인 작업, 공부 정리, AI 실험 등 매일 만든 작은 결과물을 하나씩 카드로 기록하는 공간이다.",
    note:
      "완성도보다 축적을 우선하기로 했다. 거창한 작품이 아니어도 오늘 만든 하나의 조각을 남기면, 시간이 지나 하나의 아카이브가 될 수 있다."
  },
  {
    id: 2,
    day: 2,
    date: "2026-05-26",
    title: "The Cabinet of Lost Thoughts",
    category: "Creative",
    summary: "잊힌 생각들이 사라지지 않고 작은 서랍 속에 조용히 보관되는 장면을 이미지로 만들었다.",
    thumbnailText: "Lost\nThoughts",
    image: "images/cabinet-of-lost-thoughts.png",
    description:
      "The Cabinet of Lost Thoughts는 잊힌 생각들이 완전히 사라지는 것이 아니라, 어딘가의 작은 서랍 속에 조용히 보관되고 있을지도 모른다는 상상에서 시작한 이미지 작업이다. 오래된 서랍장, 빛나는 메모 조각, 유리병과 책들은 사라진 아이디어들이 다시 발견되기를 기다리는 공간처럼 보이도록 구성했다.",
    note:
      "처음에는 인간형 존재를 중심으로 이미지를 만들었지만, 손 표현이 계속 신경 쓰였다. 그래서 존재 자체를 사물과 공간으로 바꾸었고, 오히려 Daily Pieces의 방향성과 더 잘 맞는 결과가 나왔다. 앞으로는 인간형보다 사물, 공간, 구조, 상징 중심의 이미지를 더 활용하기로 했다."
  },
  {
    id: 3,
    day: 2,
    date: "2026-05-26",
    title: "GitHub Pages 배포 완료",
    category: "Project",
    summary: "Daily Pieces 프로젝트를 GitHub에 연결하고 GitHub Pages로 배포했다.",
    thumbnailText: "GitHub\nPages",
    description:
      "오늘은 Daily Pieces 프로젝트를 GitHub 저장소에 연결하고 GitHub Pages를 통해 배포했다. 이제 작업물이 내 컴퓨터 안에만 머무는 것이 아니라, 실제 웹 주소를 통해 확인할 수 있는 형태가 되었다.",
    note:
      "작은 배포지만 프로젝트가 로컬 작업물에서 공개 가능한 웹페이지로 넘어간 기록이다. 앞으로 만든 카드와 작업물을 계속 쌓아갈 수 있는 기반이 만들어졌다."
  },
  {
    id: 4,
    day: 3,
    date: "2026-05-27",
    title: "The Museum of Tiny Errors",
    category: "Creative",
    summary: "작은 오타와 헷갈리는 기호들이 박물관 유물처럼 전시된 장면을 이미지로 만들었다.",
    thumbnailText: "Tiny\nErrors",
    image: "images/museum-of-tiny-errors.png",
    description:
      "The Museum of Tiny Errors는 아주 작은 실수들이 생각보다 긴 길을 돌아가게 만든다는 경험에서 출발한 이미지 작업이다. daily와 daliy, i와 l, 하이픈과 언더스코어처럼 사소해 보이는 차이들이 유리 진열장 속 유물처럼 보관된 박물관을 상상했다.",
    note:
      "오늘은 GitHub 저장소 이름과 remote 주소 문제를 해결하는 과정에서 작은 문자 하나가 얼마나 큰 차이를 만드는지 체감했다. 답답한 시간이었지만, 그 실수를 그냥 넘기지 않고 하나의 작업 주제로 바꾸면서 Daily Pieces의 세 번째 조각으로 남겼다."
  },
  {
    id: 5,
    day: 4,
    date: "2026-05-28",
    title: "The Garden of Sleeping Stars",
    category: "Creative",
    summary: "별이 꽃처럼 피어나고 성운이 정원처럼 펼쳐진, 고요한 우주의 정원을 이미지로 만들었다.",
    thumbnailText: "Sleeping\nStars",
    image: "images/garden-of-sleeping-stars.png",
    description:
      "The Garden of Sleeping Stars는 우주 어딘가에 조용히 잠들어 있는 별들의 정원을 상상하며 만든 이미지 작업이다. 꽃처럼 피어난 별빛, 성운이 드리운 하늘, 천문대처럼 보이는 구조물과 빛나는 식물들은 이곳이 단순한 풍경이 아니라 별들이 씨앗처럼 자라나는 장소처럼 느껴지도록 구성했다.",
    note:
      "이번 작업은 심해나 미지의 자연처럼 인간의 손이 닿지 않은 세계를 떠올리다가, 우주를 하나의 정원으로 바라보는 상상에서 출발했다. 차갑고 먼 공간으로 보이기 쉬운 우주를, 조용하고 아름답고 생명감 있는 장소처럼 표현해보고 싶었다."
  },
  {
    id: 6,
    day: 5,
    date: "2026-05-29",
    title: "The City of Many Minds",
    category: "Creative",
    summary: "수많은 사람, AI, 데이터 흐름이 하나의 거대한 신경망처럼 연결된 미래 도시를 이미지로 만들었다.",
    thumbnailText: "Many\nMinds",
    image: "images/city-of-many-minds.png",
    description:
      "The City of Many Minds는 AI 시대의 미래 도시를 상상하며 만든 이미지 작업이다. 도시의 건물, 교통망, 데이터 흐름, 에너지 네트워크가 하나의 거대한 신경망처럼 연결되어 있고, 중앙의 빛나는 코어는 수많은 지식과 판단이 모이는 집단지능의 중심처럼 보이도록 구성했다.",
    note:
      "이번 작업은 AI가 인간을 완전히 대체한다기보다, AI를 잘 활용하는 개인과 조직이 더 빠르게 실행하고 성장할 수 있다는 생각에서 출발했다. 미래 도시는 단순히 높은 건물이 많은 공간이 아니라, 사람과 기술, 데이터와 시스템이 서로 연결되어 더 빠르게 실험하고 실행하는 공간일지도 모른다."
  },
  {
    id: 7,
    day: 6,
    date: "2026-05-30",
    title: "The Archive at the End of Time",
    category: "Creative",
    summary: "시간의 끝에 남겨진 거대한 아카이브를 상상하며, 사라진 문명과 미래의 기록이 함께 보관된 공간을 이미지로 만들었다.",
    thumbnailText: "End of\nTime",
    image: "images/archive-at-end-of-time.png",
    description:
      "The Archive at the End of Time은 모든 시간이 지나간 뒤에도 남아 있는 마지막 기록 보관소를 상상하며 만든 이미지 작업이다. 거대한 서가, 떠다니는 문서함, 빛나는 두루마리, 모래시계, 별빛으로 이어진 먼 풍경이 사라진 문명과 미래의 데이터가 함께 보관된 공간처럼 느껴지도록 구성했다.",
    note:
      "오늘은 시간의 끝에 남겨진 아카이브라는 장면에 마음이 끌렸다. 매일 남기는 작은 작업은 당장 거창해 보이지 않아도, 시간이 지나면 하나의 기록이 되고 오래 남는 흔적이 된다. 이 이미지는 그런 조각들이 시간의 끝에서도 조용히 보관되어 있는 모습을 상상하며 만든 작업이다."
  },
  {
    id: 8,
    day: 7,
    date: "2026-05-31",
    title: "The Lake That Holds the Moon",
    category: "Creative",
    summary: "고요한 밤의 호수 위에 실제 크기에 가까운 보름달이 은은하게 비치는 장면을 이미지로 만들었다.",
    thumbnailText: "Moonlit\nLake",
    image: "images/moonlit-lake.png",
    description:
      "The Lake That Holds the Moon은 밤하늘의 달이 조용한 호수 위에 비치는 장면을 상상하며 만든 이미지 작업이다. 과장된 판타지 달보다 실제로 밤하늘에서 바라보는 달의 크기에 가까운 느낌을 살리고, 잔잔한 물결과 먼 산, 숲의 실루엣이 달빛 아래 차분하게 이어지도록 구성했다.",
    note:
      "오늘은 실제로 본 보름달처럼 뜬 달이 인상에 남아 달을 주제로 정했다. 처음에는 달이 너무 크게 표현되어 다시 조정했고, 최종적으로는 밤하늘에 자연스럽게 떠 있는 달과 그 빛이 호수에 조용히 번지는 장면을 남겼다."
  },
  {
    id: 9,
    day: 8,
    date: "2026-06-01",
    title: "A Well-Balanced Study Day",
    category: "Study",
    summary: "05.31 공부 기록을 바탕으로, 통계학·Vue·CodeTree·영어 루틴·Daily Pieces까지 균형 있게 진행한 하루를 정리했다.",
    thumbnailText: "Study\n9/10",
    image: "images/study-record-9-out-of-10.png",
    description:
      "A Well-Balanced Study Day는 05.31 공부 기록을 Daily Pieces 카드로 정리한 작업이다. 통계학 상관분석 PDF와 개념 문제, 상관계수 계산 및 검정 문제를 확인했고, Vue checkbox/radio 핵심 문법 복습, CodeTree, 영어 루틴, Daily Pieces, 웹강 퀴즈, 집 청소까지 마무리했다. 단순히 많은 일을 한 날이라기보다, 막히는 부분을 실제로 정리하고 컨디션을 고려해 적절히 멈춘 날로 기록했다.",
    note:
      "오늘 카드는 Creative 이미지가 아니라 공부 기록을 주제로 삼았다. Daily Pieces가 이미지 작업만 쌓는 공간이 아니라, 공부·프로젝트·회고·작은 성취까지 남기는 아카이브라는 점을 다시 확인한 날이다. 05.31 최종 점수는 9/10으로 정리했고, 꾸준히 쌓아가는 공부 흐름을 하나의 조각으로 남겼다."
  },
  {
    id: 10,
    day: 9,
    date: "2026-06-02",
    title: "The Forest of Depth-First Paths",
    category: "Creative",
    summary: "그래프 탐색 개념에서 떠올린 장면을 바탕으로, 빛나는 길들이 깊게 이어지는 신비로운 숲 이미지를 만들었다.",
    thumbnailText: "Depth-\nFirst Forest",
    image: "images/forest-of-depth-first-paths.png",
    description:
      "The Forest of Depth-First Paths는 오늘 배운 그래프, DFS, BFS, DAG, 위상 순서 개념에서 출발한 이미지 작업이다. 그중에서도 DFS의 ‘한 길을 끝까지 따라가고, 막히면 다시 돌아와 다른 길로 나아가는 흐름’을 숲 속 탐험 장면으로 바꾸어 표현했다. 어두운 숲 사이로 빛나는 길과 연결 지점들이 그래프의 정점과 간선처럼 이어지고, 전체 풍경은 하나의 거대한 탐색 구조처럼 보이도록 구성했다.",
    note:
      "오늘은 공부한 알고리즘 개념을 그대로 정리하는 대신, 그것에서 떠오른 이미지를 자유 주제로 풀어냈다. 특히 숲이라는 장면에 끌렸고, DFS의 탐험 감각이 신비로운 길과 연결 구조로 자연스럽게 이어졌다. 내일부터는 시리즈물 방향도 다시 논의해볼 예정이지만, 오늘은 우선 그래프 개념이 녹아든 자유 이미지 한 장을 Daily Pieces의 9일차 기록으로 남겼다."
  },
  {
    id: 11,
    day: 10,
    date: "2026-06-03",
    title: "The Gate of the Silent Tower",
    category: "Creative",
    summary: "탑 등반 시리즈의 시작으로, 안개 속 거대한 탑 입구와 첫 번째 봉인된 문을 담은 이미지를 만들었다.",
    thumbnailText: "Silent\nTower",
    image: "images/gate-of-the-silent-tower.png",
    description:
      "The Gate of the Silent Tower는 새롭게 시작하는 탑 등반 시리즈의 첫 장면이다. 안개에 둘러싸인 거대한 탑과 그 아래 봉인된 문은, 이제 막 긴 여정이 시작된다는 분위기를 담고 있다. 차가운 돌벽, 희미하게 빛나는 문양, 어둡고 웅장한 하늘을 통해 탑 전체가 오래된 시험의 장소처럼 보이도록 구성했다.",
    note:
      "이번 작업은 앞으로 8~10일 정도 이어갈 탑 등반 시리즈의 1화에 해당한다. 각 층마다 고유한 공간과 그 층을 지키는 수호자가 있는 구조로 진행할 생각이며, 오늘은 그 출발점인 탑의 입구를 먼저 기록했다. 복잡한 설정을 깊게 짜기보다는, 직관적으로 이어가기 쉬운 시리즈 흐름을 만드는 데 초점을 두었다."
  },
  {
    id: 12,
    day: 11,
    date: "2026-06-04",
    title: "The Silent Tower - Episode 02: The First Guardian",
    category: "Creative",
    summary: "침묵의 탑 시리즈 2화로, 탑 내부의 첫 번째 홀에서 거대한 수호자와 마주하는 장면을 이미지로 만들었다.",
    thumbnailText: "First\nGuardian",
    image: "images/silent-tower-episode-02-first-guardian.png",
    description:
      "이번 작업은 침묵의 탑 시리즈의 두 번째 장면이다. 탑의 문을 지나 내부로 들어선 뒤, 첫 번째 층의 중심 홀에서 거대한 수호자와 마주하는 순간을 표현했다. 높은 천장에서 내려오는 빛, 차가운 석조 기둥, 사슬과 등불이 걸린 어두운 공간, 그리고 묵직한 무기를 든 갑옷형 수호자를 중심으로 배치해 첫 전투 직전의 긴장감을 담았다.",
    note:
      "1화가 탑의 입구와 시작의 분위기를 보여주는 장면이었다면, 2화는 이 시리즈의 핵심 방향인 '각 층마다 고유한 수호자가 존재한다'는 설정을 본격적으로 드러내는 조각이다. 너무 과한 스케일보다는 실제로 눈앞에서 마주친 듯한 위압감에 집중했고, 앞으로도 층이 올라갈수록 공간과 수호자의 개성이 점점 강해지는 흐름으로 이어갈 예정이다."
  },
  {
    id: 13,
    day: 11,
    date: "2026-06-04",
    title: "Moonlit Sleepy Cat",
    category: "Creative",
    summary: "달빛 아래 잠든 고양이를 통해 피곤한 밤, 그냥 푹 자고 싶은 마음을 이미지로 남겼다.",
    thumbnailText: "Sleepy\nCat",
    image: "images/moonlit-sleepy-cat.png",
    description:
      "오늘은 무언가 거창한 장면보다, 피곤한 상태에서 자연스럽게 떠오른 감정을 담아 보고 싶었다. 달빛이 비치는 밤, 포근한 쿠션 위에서 잠든 고양이는 지금의 내 마음처럼 아무 생각 없이 쉬고 싶다는 감정을 상징한다. 전체적으로는 너무 어둡지 않게, 부드럽고 편안한 분위기가 느껴지도록 구성했다.",
    note:
      "오늘은 꽤 피곤했지만, 그 상태 자체를 하나의 주제로 바꾸니 오히려 작업이 더 자연스럽게 이어졌다. 그냥 자고 싶다는 단순한 감정도 Daily Pieces에서는 충분히 하나의 조각이 될 수 있다는 걸 보여준 작업이다."
  },
  {
    id: 14,
    day: 12,
    date: "2026-06-05",
    title: "The Silent Tower - Episode 03: The Lantern Serpent",
    category: "Creative",
    summary: "침묵의 탑 시리즈 3화로, 어두운 계단 홀을 감싸는 등불 뱀 수호자를 이미지로 만들었다.",
    thumbnailText: "Lantern\nSerpent",
    image: "images/silent-tower-episode-03-lantern-serpent.png",
    description:
      "The Silent Tower - Episode 03: The Lantern Serpent는 침묵의 탑 시리즈의 세 번째 장면이다. 첫 번째 수호자였던 갑옷형 존재와 달리, 이번에는 긴 몸을 가진 뱀 또는 용 형태의 수호자가 탑 내부의 거대한 홀과 계단을 감싸고 있는 모습을 표현했다. 어두운 석조 공간, 매달린 등불, 희미한 빛이 번지는 바닥을 통해 너무 밝지는 않지만 장면이 잘 보이는 어두운 판타지 분위기를 만들었다.",
    note:
      "처음 생성한 이미지는 분위기는 좋았지만 너무 어두워서 세부 구조가 잘 보이지 않았다. 이후 전체적인 어두운 톤과 뱀 수호자 구조는 유지하되, 등불과 공간의 빛을 조금 더 살려서 시야가 확보되는 방향으로 다시 조정했다. 2화의 갑옷 수호자와 겹치지 않도록 이번 화는 비인간형 수호자로 방향을 바꾼 점이 핵심이다."
  },
  {
    id: 15,
    day: 13,
    date: "2026-06-06",
    title: "The Silent Tower - Episode 04: The Mirror-Winged Beast",
    category: "Creative",
    summary: "침묵의 탑 시리즈 4화로, 깨진 거울의 방에서 유리 날개를 펼친 거울 수호자를 이미지로 만들었다.",
    thumbnailText: "Mirror\nBeast",
    image: "images/silent-tower-episode-04-mirror-winged-beast.png",
    description:
      "The Silent Tower - Episode 04: The Mirror-Winged Beast는 침묵의 탑 시리즈의 네 번째 장면이다. 이번 층은 깨진 거울과 반사된 빛으로 가득한 방이며, 그 중심에는 검은 날개와 유리 파편이 합쳐진 거울 수호자가 서 있다. 어두운 톤은 유지하되, 창문과 거울 조각에서 들어오는 차가운 빛을 통해 수호자의 형태와 공간 구조가 잘 보이도록 구성했다.",
    note:
      "처음에는 여러 버전의 거울 수호자 이미지를 생성했지만, 최종적으로 정면 구도에서 수호자의 형태가 가장 또렷하게 보이는 이미지를 선택했다. 3화의 등불 뱀 수호자와 다르게, 4화는 거울 조각과 날개를 중심으로 한 차가운 분위기의 수호자로 방향을 바꾸었다. 너무 밝지도, 너무 어둡지도 않은 톤을 맞추는 것이 이번 작업의 핵심이었다."
  },
  {
    id: 16,
    day: 14,
    date: "2026-06-10",
    title: "The Silent Tower - Episode 05: The Chronomancer Warden",
    category: "Creative",
    summary: "침묵의 탑 시리즈 5화로, 시계 장치와 고차원적인 마법을 다루는 중간 보스를 이미지로 만들었다.",
    thumbnailText: "Chrono\nWarden",
    image: "images/silent-tower-episode-05-chronomancer-warden.png",
    description:
      "The Silent Tower - Episode 05: The Chronomancer Warden는 침묵의 탑 시리즈의 다섯 번째 장면이자 중간 보스에 해당하는 이미지다. 이번 층은 거대한 시계 장치와 기계 구조물로 가득한 성당 같은 공간이며, 그 중심에는 시간을 다루는 듯한 고차원적인 존재가 떠 있다. 기계 장치, 원형 마법진, 푸른 빛과 금속 구조가 함께 어우러지며, 단순한 기계 수호자가 아니라 마법사에 가까운 초월적 중간 보스의 분위기를 담도록 구성했다.",
    note:
      "처음에는 기계 기사나 자동 병기 같은 방향도 시도했지만, 이번 5화는 중간 보스인 만큼 조금 더 특별하고 상위 존재 같은 느낌이 필요하다고 판단했다. 그래서 시계 장치와 마법이 결합된 고차원적 수호자라는 방향으로 조정했고, 최종적으로 마법사 같은 실루엣과 시간/기계 구조가 함께 보이는 이미지를 선택했다. 이 장면은 탑 시리즈가 본격적으로 더 높은 단계로 넘어가는 분기점 역할을 한다."
  },
  {
    id: 17,
    day: 15,
    date: "2026-06-11",
    title: "The Silent Tower - Episode 06: The Ashen Dragon of the Broken Constellation",
    category: "Creative",
    summary: "침묵의 탑 시리즈 6화로, 부서진 별자리와 천체 고리 속에 나타난 잿빛 용 수호자를 이미지로 만들었다.",
    thumbnailText: "Ashen\nDragon",
    image: "images/silent-tower-episode-06-ashen-dragon-broken-constellation.png",
    description:
      "The Silent Tower - Episode 06: The Ashen Dragon of the Broken Constellation은 침묵의 탑 시리즈의 여섯 번째 장면이다. 중간 보스였던 시계 마법사 이후, 탑의 상층부는 단순한 성당이나 전투 공간을 넘어 별자리와 천체 구조가 무너진 고차원적인 공간으로 확장된다. 그 중심에는 재와 돌, 금빛 별자리 조각이 뒤섞인 거대한 용 수호자가 서 있으며, 부서진 천체 고리와 별의 선들이 용의 몸 주변을 감싸고 있다.",
    note:
      "처음에는 일반적인 잿빛 드래곤과 폐허 성당 느낌으로 이미지를 생성했지만, 생각보다 평범한 드래곤 장면처럼 보여 마음에 들지 않았다. 그래서 5화 이후부터는 조금 더 고차원적인 분위기를 넣는 방향으로 수정했고, 최종적으로 별자리와 천체 고리가 결합된 Ashen Dragon 이미지를 선택했다. 이미지 안에 들어간 글자는 제거해, Daily Pieces 카드에 더 자연스럽게 들어가도록 정리했다."
  },
  {
    id: 18,
    day: 16,
    date: "2026-06-12",
    title: "Algorithm Shorts Lab - Bubble Sort Prototype",
    category: "Project",
    summary: "Python과 matplotlib로 첫 번째 알고리즘 시각화 GIF인 Bubble Sort 쇼츠 프로토타입을 완성했다.",
    thumbnailText: "Bubble\nSort",
    image: "images/algorithm-shorts-lab-bubble-sort-prototype.png",
    description:
      "Algorithm Shorts Lab - Bubble Sort Prototype은 알고리즘을 세로형 쇼츠 콘텐츠로 시각화하는 첫 번째 실험이다. Python과 matplotlib를 사용해 Bubble Sort 과정을 9:16 비율의 GIF로 만들었고, 막대 그래프, 비교와 교환 색상, C++ 핵심 코드 패널, PASS / COMPARE / SWAP 카운터를 함께 보여주도록 구성했다. 아직 MP4 변환과 배경음악 추가는 하지 않았지만, 알고리즘 학습과 콘텐츠 제작을 연결하는 첫 번째 완성 프로토타입이라는 점에서 의미가 있다.",
    note:
      "이번 작업은 Codex를 활용해 전체 코드를 구성하고, 여러 번 결과물을 확인하면서 디자인과 애니메이션을 다듬은 프로젝트다. 검은 배경, 숫자가 표시된 막대, 비교/교환/정렬 상태의 색상 구분, 코드 라인 하이라이트, 완료 장면까지 포함해 하나의 짧은 알고리즘 시각화 콘텐츠 형태를 만들었다. 기말이 완전히 끝난 뒤에는 이 GIF를 MP4로 변환하고 배경음악을 추가해 실제 Shorts 콘텐츠로 발전시킬 계획이다."
  },
  {
    id: 19,
    day: 16,
    date: "2026-06-12",
    title: "The Silent Tower - Episode 07: The Starless Judge",
    category: "Creative",
    summary: "침묵의 탑 시리즈 7화로, 별빛이 사라진 최상층에서 최종 관문을 지키는 심판자 이미지를 만들었다.",
    thumbnailText: "Starless\nJudge",
    image: "images/silent-tower-episode-07-the-starless-judge.png",
    description:
      "The Silent Tower - Episode 07: The Starless Judge는 침묵의 탑 시리즈의 일곱 번째 장면이다. 이번 화는 최종 보스 직전의 관문에 해당하며, 별빛이 사라진 최상층 공간에서 거대한 심판자와 마주하는 순간을 표현했다. 검은 월식 같은 원형 구조와 부서진 천체 고리, 떠다니는 파편들, 긴 계단 끝에 서 있는 심판자의 실루엣을 통해 고차원적이고 압도적인 분위기를 담았다. 전체적으로는 어두운 톤을 유지하되, 너무 어둡지 않게 구조와 형상이 잘 보이도록 조정했다.",
    note:
      "처음에는 7화 수호자 이미지를 여러 버전으로 시도했고, 그중 세 번째 이미지를 기준으로 다시 변형 이미지를 생성했다. 최종적으로는 월식처럼 보이는 검은 원형 구조와 거대한 심판자의 실루엣이 가장 인상적으로 드러나는 이미지를 선택했다. 6화의 별자리와 천체 고리 분위기를 이어받으면서도, 이번 화는 별빛이 사라진 공허와 최종 관문 특유의 정적인 압박감을 더 강조한 장면이다."
  },
  {
    id: 20,
    day: 17,
    date: "2026-06-13",
    title: "The Heart of the Silent Tower",
    category: "Creative",
    summary: "침묵의 탑 시리즈의 최종화로, 푸른 심장을 품은 거대한 천상형 존재가 탑의 중심에서 모습을 드러내는 장면을 담았다.",
    thumbnailText: "Tower\nHeart",
    image: "images/heart-of-the-silent-tower.png",
    description:
      "The Heart of the Silent Tower는 침묵의 탑 시리즈의 마지막 장면으로, 탑의 가장 깊은 중심부에서 마주하는 핵심 존재를 표현한 이미지 작업이다. 푸른 심장을 품은 거대한 천상형 존재는 단순한 수호자가 아니라, 이 탑 전체를 움직여 온 근원 그 자체처럼 보이도록 구성했다. 거대한 날개, 빛나는 중심핵, 별빛이 스며든 성당형 공간은 시리즈의 마지막에 어울리는 장엄함과 초월적인 분위기를 담아낸다.",
    note:
      "이번 작품은 침묵의 탑 시리즈의 최종화이자 마무리 조각이다. 이전 화들이 각 층의 수호자와 중간 보스를 보여줬다면, 마지막에는 탑의 중심과 본질에 가까운 존재를 그리는 방향으로 정리했다. 전체 시리즈를 돌아봤을 때 어둡고 신비로운 분위기, 성당 같은 공간감, 그리고 점점 고차원적으로 커지는 스케일이 하나의 흐름으로 이어졌다는 점이 특히 마음에 든다."
  },
  {
    id: 21,
    day: 18,
    date: "2026-06-14",
    title: "The Relic Hall of the Silent Tower",
    category: "Creative",
    summary: "침묵의 탑 각 층에서 얻은 전리품과 유물들이 한곳에 보관된 에필로그 장면을 이미지로 만들었다.",
    thumbnailText: "Relic\nHall",
    image: "images/silent-tower-relic-hall.png",
    description:
      "The Relic Hall of the Silent Tower는 탑의 각 층을 지나며 마주했던 수호자들과 전투의 흔적이 전리품과 유물의 형태로 보관된 공간을 상상하며 만든 이미지 작업이다. 기사 갑옷과 창, 뱀의 유물, 수정 날개, 기계 장치, 별자리의 힘이 깃든 구체, 보랏빛 왕관 같은 전리품들이 거대한 성당형 전시실 안에 정렬되어 있으며, 탑 시리즈 전체를 돌아보는 에필로그의 분위기를 담고 있다.",
    note:
      "전투가 끝난 뒤 남은 것은 단순한 승리가 아니라, 각 층에서 마주한 존재들의 흔적과 기록이라는 느낌으로 정리했다. 이 이미지는 침묵의 탑 시리즈를 하나의 회고이자 보관소처럼 마무리해 주는 장면이다."
  },
  {
    id: 22,
    day: 19,
    date: "2026-06-15",
    title: "A Room Where Thoughts Settle",
    category: "Creative",
    summary: "생각들이 천천히 정리되고 가라앉는 조용한 방의 분위기를 이미지로 만들었다.",
    thumbnailText: "Thoughts\nSettle",
    image: "images/room-where-thoughts-settle.png",
    description:
      "A Room Where Thoughts Settle은 복잡하게 떠다니던 생각들이 조용한 공간 안에서 천천히 정리되는 순간을 상상하며 만든 이미지 작업이다. 따뜻한 조명 아래 놓인 책상, 펼쳐진 노트, 머그컵, 책장, 그리고 공중에 떠오르는 종이 조각들은 아이디어와 감정이 서서히 자리를 찾아가는 흐름처럼 보이도록 구성했다. 전체적으로는 포근하고 차분한 분위기를 유지하면서도, 일상적인 작업 공간이 조금은 마법적인 사유의 방처럼 느껴지도록 표현했다.",
    note:
      "오늘은 침묵의 탑 시리즈 이후 가볍게 이어갈 수 있는 자유주제로 작업했다. 거대한 세계관이나 강한 사건보다, 조용한 방 안에서 생각이 정리되는 느낌 자체를 하나의 장면으로 남기고 싶었다. 떠다니는 종이 조각은 흩어진 생각들이 천천히 가라앉는 흐름을 상징하며, 이번 작업은 Daily Pieces의 일상적이고 편안한 방향을 다시 보여주는 조각이 되었다."
  },
  {
    id: 23,
    day: 20,
    date: "2026-06-16",
    title: "The Valley of Sleeping Giants",
    category: "Creative",
    summary: "잠든 거대한 존재들의 흔적이 계곡 전체를 이루는 환상적인 풍경을 이미지로 만들었다.",
    thumbnailText: "Sleeping\nGiants",
    image: "images/sleeping-giants-valley.png",
    description:
      "The Valley of Sleeping Giants는 오래전 잠든 거인들이 산과 계곡의 일부처럼 남아 있는 세계를 상상하며 만든 이미지 작업이다. 거대한 얼굴과 손, 이끼와 식물로 뒤덮인 유적, 물줄기와 계곡의 흐름이 한 장면 안에 어우러지며 고요하면서도 장대한 분위기를 만든다. 자연과 거인의 흔적이 하나로 합쳐진 풍경을 통해, 잊힌 존재들의 시간이 아직도 이 땅에 남아 있다는 느낌을 표현했다.",
    note:
      "오늘은 일상적인 분위기보다 미지의 세계를 탐험하는 듯한 자유주제로 작업했다. 이 장면은 단순한 풍경이라기보다, 거대한 존재가 세계의 일부가 된 뒤 오랜 시간이 지난 모습을 떠올리며 구성했다. 거인의 흔적이 폐허나 유물이 아니라 지형 자체처럼 보이도록 하는 점이 핵심이었고, 고요한 아름다움과 웅장함을 함께 담고 싶었다."
  },
  {
    id: 24,
    day: 20,
    date: "2026-06-16",
    title: "The Sea of Floating Ruins",
    category: "Creative",
    summary: "하늘 위에 떠 있는 폐허와 부유 섬들이 이어진 미지의 세계를 이미지로 만들었다.",
    thumbnailText: "Floating\nRuins",
    image: "images/sea-of-floating-ruins.png",
    description:
      "The Sea of Floating Ruins는 구름 바다 위에 떠 있는 고대 폐허와 부유 섬들로 이루어진 세계를 상상하며 만든 이미지 작업이다. 공중에 흩어진 유적과 다리, 푸른 수정과 빛, 거대한 구조물들이 층층이 펼쳐지며 탐험의 욕구를 자극하는 장면으로 구성했다. 하늘 자체가 바다처럼 느껴지는 공간 안에서, 잊힌 문명이 아직도 어딘가 살아 숨 쉬는 듯한 분위기를 표현했다.",
    note:
      "오늘 두 번째 카드는 보다 직접적으로 모험과 탐험의 느낌을 담고 싶어서 선택했다. 떠 있는 섬과 폐허, 강한 푸른빛과 열린 하늘 구조가 미지의 세계라는 인상을 잘 만들어 준다고 판단했다. 탑 시리즈 이후에도 이런 식의 환상적이고 우주적인 자유주제를 가볍게 이어갈 수 있겠다는 가능성을 보여준 작업이다."
  },
  {
    id: 25,
    day: 21,
    date: "2026-06-22",
    title: "The Whale That Carries a City",
    category: "Creative",
    summary: "거대한 고래의 등에 하나의 도시가 세워진 환상적인 우주 풍경을 이미지로 만들었다.",
    thumbnailText: "City\nWhale",
    image: "images/whale-that-carries-a-city.png",
    description:
      "The Whale That Carries a City는 하늘과 우주 사이를 유영하는 거대한 고래 위에 하나의 찬란한 도시가 세워진 세계를 상상하며 만든 이미지 작업이다. 푸른 별빛과 황금빛 구름, 멀리 떠 있는 부유 도시들, 그리고 고래의 등 위에 층층이 쌓인 궁전과 탑들이 함께 어우러져 장대한 판타지 풍경을 만든다. 생명체와 문명이 하나의 존재처럼 결합된 장면을 통해, 미지의 세계를 여행하는 듯한 경이로움과 스케일감을 표현했다.",
    note:
      "오늘은 자유주제 후보들 중에서도 가장 강하게 끌렸던 장면을 선택했다. 단순히 거대한 동물 위에 도시를 올리는 발상을 넘어서, 살아 있는 존재 자체가 하나의 세계가 된 느낌을 담고 싶었다. 우주적 분위기와 동화적인 장대함이 잘 섞여서, 이번 작업은 Daily Pieces에서 환상적 상상력을 확장하는 좋은 한 장면이 되었다."
  },
  {
    id: 26,
    day: 22,
    date: "2026-06-29",
    title: "The Road Through the Summer Field",
    category: "Creative",
    summary: "여름 들판 사이로 이어지는 길과 맑은 하늘을 담은 밝고 평화로운 풍경 이미지를 만들었다.",
    thumbnailText: "Summer\nField",
    image: "images/road-through-summer-field.png",
    description:
      "The Road Through the Summer Field는 넓은 여름 들판 사이로 이어지는 흙길을 따라 조용히 앞으로 나아가는 느낌을 담은 이미지 작업이다. 푸른 하늘과 흰 구름, 길가의 야생화, 초록빛 들판과 나무가 어우러지며 밝고 개방적인 분위기를 만든다. 거대한 판타지나 우주적 장면이 아니라, 자연 속의 단순한 길 하나를 통해 여름의 여유와 앞으로 이어질 하루의 흐름을 표현했다.",
    note:
      "오늘은 자유주제를 다시 진행하되, 우주와 판타지에 치우치지 않고 여러 방향의 후보를 섞어서 살펴봤다. 그중 가장 마음에 든 것은 여름 들판을 가로지르는 길의 이미지였다. 이전에 생성했던 자연 풍경 이미지는 등록하지 않았고, 이번 작업은 6월 29일의 새로운 카드로 정리했다. 밝고 시원한 분위기가 최근의 무거운 이미지들과 대비되어 좋은 전환점이 되었다."
  },
  {
    id: 27,
    day: 23,
    date: "2026-06-30",
    title: "The Forest Where Rain Still Falls",
    category: "Creative",
    summary: "비가 막 지나간 듯한 깊은 숲길과 젖은 공기를 담은 자연 풍경 이미지를 만들었다.",
    thumbnailText: "Rain\nForest",
    image: "images/forest-where-rain-still-falls.png",
    description:
      "The Forest Where Rain Still Falls는 비가 스며든 숲속 오솔길의 차갑고 촉촉한 분위기를 담아낸 이미지 작업이다. 젖은 흙길과 이끼 낀 바위, 빗물을 머금은 나뭇잎, 그리고 흐릿한 빛이 스며드는 깊은 숲이 함께 어우러져 조용하고 몰입감 있는 자연 풍경을 만든다. 화려한 환상성보다는 실제 숲속에 들어와 있는 듯한 공기와 습도, 그리고 비가 남긴 고요함을 중심으로 표현했다.",
    note:
      "오늘은 여러 장르의 자유주제 후보를 한꺼번에 생성해 보고 그중에서 선택했다. 최종적으로는 화려한 판타지보다는, 비가 내린 뒤의 깊은 숲길이 주는 고요한 분위기가 가장 마음에 들었다. 최근 우주적이거나 상징적인 장면이 많았던 흐름에서 잠시 벗어나, 자연의 촉감과 공기를 느낄 수 있는 이미지로 정리한 점이 특히 좋았다."
  },
  {
    id: 28,
    day: 24,
    date: "2026-07-02",
    title: "The Table of Hearts and Fortune",
    category: "Creative",
    summary: "하트, 카드, 행운의 상징이 가득 놓인 로맨틱한 판타지 게임 테이블 장면을 이미지로 만들었다.",
    thumbnailText: "Heart\nTable",
    image: "images/heart-fortune-card-table.png",
    description:
      "The Table of Hearts and Fortune은 화려한 카드와 하트 장식, 클로버, 리본, 꽃, 따뜻한 오후의 빛이 한 테이블 위에 모인 로맨틱한 판타지 이미지 작업이다. 카드 게임의 분위기와 점술 테이블 같은 신비로운 느낌, 그리고 사랑과 행운을 기념하는 듯한 따뜻한 감성이 함께 담겨 있다. 분홍빛과 금빛, 붉은색이 어우러진 장면을 통해 평범한 테이블을 애정과 행운, 놀이의 즐거움이 모이는 작은 마법 공간처럼 표현했다.",
    note:
      "이번에는 사랑, 카드, 행복 같은 단어에서 이어지는 분위기를 바탕으로 이미지를 골랐다. 여러 장면 중에서도 이 이미지는 하트와 카드, 장식 소품들이 한 테이블 위에 조화롭게 모여 있어서 주제가 가장 선명하게 느껴졌다. 단순한 카드놀이 장면이라기보다 사랑과 행운, 따뜻한 감정이 함께 담긴 판타지 테이블처럼 보여서 오늘의 Daily Pieces로 정리하기 좋았다."
  },
  {
    id: 29,
    day: 25,
    date: "2026-07-03",
    title: "The Cosmic Arcade Festival",
    category: "Creative",
    summary: "우주 공간에 열린 화려한 아케이드 축제를 네온빛과 별빛, 귀여운 캐릭터들로 표현한 이미지를 만들었다.",
    thumbnailText: "Cosmic\nArcade",
    image: "images/cosmic-arcade-festival.png",
    description:
      "The Cosmic Arcade Festival은 우주 공간에 열린 환상적인 아케이드 축제를 상상하며 만든 이미지 작업이다. 별과 행성, 은하가 떠 있는 밤하늘 아래에 네온빛 게임기와 반짝이는 장식, 별 모양 조명, 작은 캐릭터들이 가득한 축제 공간이 펼쳐져 있다. 게임장의 활기와 우주적 분위기, 귀여운 판타지 감성이 함께 섞이면서 밝고 화려한 하루의 장면처럼 느껴지도록 표현했다.",
    note:
      "오늘은 시원함, 책, 전투, 게임, 환상, 축제, 예쁨, 우주 같은 키워드를 바탕으로 자유주제를 진행했다. 생성된 이미지 중 괜찮은 후보가 여러 개 있었고, 그중에서도 우주 아케이드 축제 이미지는 색감과 분위기가 가장 즉각적으로 눈에 들어왔다. 네온빛과 별빛, 게임 축제의 즐거움이 잘 살아 있어서 7월 3일 카드로 선택했다. 남은 좋은 후보들은 내일 작업에도 이어서 활용할 수 있을 것 같다."
  },
  {
    id: 30,
    day: 26,
    date: "2026-07-07",
    title: "The Sword and the Storybook",
    category: "Creative",
    summary: "거대한 책 속 세계에서 검을 든 영웅이 튀어나오는 판타지 전투 장면을 이미지로 만들었다.",
    thumbnailText: "Sword\nStory",
    image: "images/sword-and-the-storybook.png",
    description:
      "The Sword and the Storybook은 한 권의 거대한 책 속에서 영웅과 전장이 현실처럼 펼쳐지는 장면을 상상하며 만든 이미지 작업이다. 펼쳐진 책 위에서 검을 든 주인공이 몸을 내딛고 있고, 뒤편에는 성, 용, 군세, 어두운 적의 존재가 함께 나타나며 하나의 장대한 판타지 서사가 터져 나오는 듯한 구성을 만든다. 책과 이야기, 전투와 모험이 한 장면 안에 압축되어 있어서 마치 한 권의 서사시가 눈앞에서 살아 움직이는 듯한 분위기를 표현했다.",
    note:
      "오늘은 7월 7일 기준으로 26일차 작업을 진행했다. 이전에 생성해 두었던 후보들 가운데서도 이 이미지는 책, 전투, 판타지라는 요소가 강하게 결합되어 있어서 가장 인상적으로 느껴졌다. 단순히 책을 읽는 장면이 아니라 이야기 속 인물이 현실로 튀어나오는 듯한 연출이 마음에 들었고, 한 장면만으로도 모험의 시작과 긴장감이 잘 전달되어 오늘의 Daily Pieces로 선택했다."
  }
];